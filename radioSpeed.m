...
wave_number = ang_freq * sqrt( (mi * epsilon / 2) * (sqrt(1 + (sigma/(ang_freq*epsilon))^2 ) + 1)); % returns attenuation in nepers per meterattenuation = ang_freq * sqrt( (mi * permittivity / 2) * (sqrt(1 + (conductivity / permittivity)^2 ) - 1)); % returns attenuation in nepers per meter